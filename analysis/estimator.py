...
from analysis.health import ChainHealthException
...
class OnlineExpectationEstimatorWithTrace:
    ...
    def estimate(self):
        return {
            ...
            **{
                f"std/{i}": torch.sqrt(
                    self.second_moments[i, self.least_num_samples_seen - 1]
                    - self.first_moments[i, self.num_samples_seen[i] - 1] ** 2
                ).detach()
                ...
                _estimates.append(
                    {
                        ...
                        "std": (
                            torch.sqrt(
                                self.second_moments[chain, draw] - self.first_moments[chain, draw] ** 2
                            )
                        ).detach()
                        .cpu()
                        .numpy(),
...
def get_estimator(num_chains: int, num_draws: int, observable_dim: int = 1, device="cpu", online: bool = False, include_trace: bool = False):
    if include_trace:
        ...
        raise ValueError("Estimators with trace must be online.")
    elif online:
